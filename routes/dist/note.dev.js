...
var con = require("./database"); // 添加错题记录


route.post('/add', function (req, res) {
  // const questionId = req.query.questionId;
  var questionId = JSON.parse(req.query.questionId);
  questionId.map(function (item) {
...
route["delete"]('/delete', function (req, res) {
  var sql = 'delete from wrong_note where noteId= ?'; // var sql = 'delete from wrong_note where questionId= ?and userId=?'

  var params = [req.query.noteId]; // var params = [req.query.questionId, req.query.userId]
...
route.put('/fork', function (req, res) {
  ...
  var params = [1, req.query.noteId]; // var sql = 'update wrong_note set isStar=?, where questionId=? and userId=?';
...
route.put('/unfork', function (req, res) {
  var sql = 'update wrong_note set isStar=? where noteId=?';
  var params = [0, req.query.noteId]; // var sql = 'update wrong_note set isStar=?, where questionId=? and userId=?';
  // var params = [1, req.query.questionId, req.query.userId]
...
route.put('/unlock', function (req, res) {
  var sql = 'update wrong_note set `unlock`=? where noteId=?'; // const unlock='1';
  // var sql = 'update wrong_note set unlock=?, where (questionId=? and userId=?)';

  var params = [1, req.query.noteId]; // var params = [1, req.query.questionId, req.query.userId]
