...
route.post('/addAchieve', function (req, res) {
  var feature = JSON.stringify(req.query.feature);
  var sql = 'insert into achieve (`name`, `desc`,`number`,`modeId`,`feature` ) VALUES (?, ?,?,?,?)'; // var sql = 'insert into achieve set  name=? , desc=?,number=?, modeId=?, feature=?'; 
...
route.put('/update', function (req, res) {
  var feature = JSON.stringify(req.query.feature || null);
  var name = req.query.name || '';
  var desc = req.query.desc || '';
  var number = req.query.number || null;
  var modeId = req.query.modeId || null;
  var id = req.query.id;
  var sql = 'update achieve set `name`=?, `desc`=?, `number`=?,`modeId`=?,`feature`=? where id=?';
  var params = [name, desc, number, modeId, feature, id]; // var sql = 'update achieve set name=?, `desc`=? where id=?';
